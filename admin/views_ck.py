...
@app.route('/admin/ck_upload/', methods=['GET', 'POST'])
def ck_upload():
    """
    ...
    upload_ext = os.path.splitext(upload_file.filename)[1]
    ...
    </script>""" % (request.args.get('CKEditorFuncNum'), url))
...
@app.route('/admin/ck_browse/', methods=['GET', 'POST'])
def ck_browse():
    images = get_image_browse_urls()
    return render_template('ckeditor_browse.html', images=images)
